...
namespace LoopMax::Core::Hal {
            ...
            void idf_mqtts::begin(Types::MqttsData& config) {
                _config = &config;
            ...
            void idf_mqtts::disconnect() {
                _mqtt.disconnect();
            }

            bool idf_mqtts::publish(
            ...
            bool idf_mqtts::subscribe(const std::string& topic) {
                if (!_mqtt.connected()) return false;
                return _mqtt.subscribe(topic.c_str());
            }

            void idf_mqtts::onMessage(Types::MqttCallback cb) {
                _callback = cb;
            }
            ...
            bool idf_mqtts::isConnected() {
                return _mqtt.connected();
            }

            void idf_mqtts::_internalCallback(
                char* topic,
                byte* payload,
                unsigned int length
            ) {
...
/*
#include "core/hal/arduino/mqtts/idf_mqtts.h"
